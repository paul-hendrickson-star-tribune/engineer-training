...
async function asyncLessHeck({ addFoo, addBar }) {
  /** Your code here */
  const start = "";
  const foo = await addFoo(start);
  const foobar = await addBar(foo);
  const foobarfoo = await addFoo(foobar);
  const foobarfoofoo = await addFoo(foobarfoo);
  const foobarfoofoobar = await addBar(foobarfoofoo);
  const foobarfoofoobarfoo = await addFoo(foobarfoofoobar);

  const foobarfoofoobarfoofoo = await addFoo(foobarfoofoobarfoo);
  return addBar(foobarfoofoobarfoofoo);
